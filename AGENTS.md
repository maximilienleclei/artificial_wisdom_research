...
17. For convergence or optimization runs, do not reserve a fixed tail portion of the wall-clock budget for evaluation by default. Use the full budget for optimization unless the user explicitly asks for an in-run evaluation split; closed-loop evaluation can be run separately afterward.
